...
from public.utils import tables, redis
...
class WorkerFacade(object):
    ...
    def process_share(extra):
        ...
        rows = ThriftHBaseStorage.get_instance().fetch(tables.TABLE_FUND, tables.TABLE_VISITED, redis.get_visited(),
                                                       '=', columns)
        # logger.debug(rows)
        if not rows:
            redis.set_reverse()
        ...
        if rows:
            ...
            ThriftHBaseStorage.get_instance().update(tables.TABLE_FUND, rows[0].row, {redis.visited: redis.unvisited if redis.get_visited() == redis.visited else redis.visited})

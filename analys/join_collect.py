...
if __name__ == "__main__":
    ...
    grade = sqlContext.sql("select code, master, grade, name from hbase_stock_new_grade where date='{0}'".format(tools.day_after_now(-1))).map(lambda row: Row(code=row.code, grade=row.grade, name=row.name))
    # sqlContext.registerRDDAsTable(grade, "grade")
    gradeSchema = otherContext.inferSchema(grade)
    gradeSchema.registerTempTable("grade")
    stocks = sqlContext.sql("select * from hbase_stock where split(rowkey, '_')[1] > '{0}'".format(tools.day_after_now(-4))).map(lambda row: (row.code, row)).groupByKey().filter(two_decreace).map(sum_decrease_info).filter(lambda row: row[1] <= -0.0).map(lambda row: Row(code=row[0], delta=row[1]))
